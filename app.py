...
st.markdown("A data analytics project analyzing IPL T20 matches using ball-by-ball data to uncover tactical insights beyond traditional stadium averages.")

# load the data once and then it can be retrieved from the cache repeatedly whenever needed 
@st.cache_data
def load_data():
...
sns.barplot(data=melted_data, x='Match Phase', y='Average Percentage of Total Score',
            hue='outcome', palette=['#ff4d4d', '#2ecc71'], ax=ax)
...
ax[0].bar_label(bars_batters, fmt='%.2f', padding=5, fontsize=12, fontweight='bold', color='#2c3e50')
...
# VENUE PHASE ANALYSIS — BATTING PITCHES
st.header("🏟️ Venue Specific Phase Analysis")
...
st.info("**Key Insight:** Every coach talks about avoiding “wickets in clusters,” but this project tries to measure exactly how a collapse unfolds after a dismissal. The Wicket Cascade Index compares a team’s normal scoring pace with their scoring rate immediately after losing a wicket to understand how much momentum is lost. This helps identify which teams stay composed under pressure and which batting lineups are more vulnerable to sudden collapses.")

# CHOKE HOLD METRIC — DOT BALL HEATMAP
